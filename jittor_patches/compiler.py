...
if use_data_gz:
    ...
    if target_md5 != md5:
        ...
        # macOS clang PMF cast fix: preprocess, fix casts, compile preprocessed
        import platform as _plat
        if _plat.system() == "Darwin":
            import re as _re
            _pp_path = data_s_path + ".pp.cc"
            run_cmd(fix_cl_flags(f"\"{cc_path}\" -E {dflags} -include \"{vdp}\" \"{data_s_path}\" -o \"{_pp_path}\""))
...
"""
            _ns_idx = _pp.find('namespace jittor {')
            if _ns_idx >= 0:
                _brace = _pp.find('{', _ns_idx) + 1
                _pp = _pp[:_brace] + _helper + _pp[_brace:]
            _pp, _cnt = _re.subn(
                r'\(void\(\s*\*\s*\)\(Node\s*\*\s*\)\)\s*&\s*Node\s*::\s*(\w+)',
                r'unsafe_pmf_cast<void(*)(Node*)>(&Node::\1)',
                _pp)
            with open(_pp_path, 'w') as _f:
                _f.write(_pp)
            run_cmd(fix_cl_flags(f"\"{cc_path}\" {dflags} \"{_pp_path}\" -c -o \"{data_o_path}\""))
            os.remove(_pp_path)
        else:
